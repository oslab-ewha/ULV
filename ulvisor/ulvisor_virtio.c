...
#define virtio_panic(msg, ...) do {					\
		printf("ulvisor virtio error:" msg, ##__VA_ARGS__);	\
		exit(1);						\
	} while (0)
...
static inline void
virtio_set_avail_event(struct virtio_queue *q, uint16_t val)
{
	*((uint16_t *)&q->used->ring[q->num]) = val;
}

static inline void
virtio_deliver_irq(struct virtio_dev *dev)
{
...
static inline uint16_t
virtio_get_used_idx(struct virtio_queue *q)
{
	return le16toh(q->used->idx);
}

static inline void
virtio_add_used(struct virtio_queue *q, uint16_t used_idx, uint16_t avail_idx, uint16_t len)
{
...
virtio_req_complete(struct virtio_req *req, uint32_t len)
{
	 ...
	 */
	if (send_irq || vring_need_event(le16toh(virtio_get_used_event(q)),
...
vring_desc_at_le_idx(struct virtio_queue *q, __virtio16 le_idx)
{
	return &q->desc[le16toh(le_idx) & (q->num -1)];
}

static inline struct vring_desc *
vring_desc_at_avail_idx(struct virtio_queue *q, uint16_t idx)
{
...
static void
add_dev_buf_from_vring_desc(struct virtio_req *req, struct vring_desc *vring_desc)
{
	...
	if (!(buf->iov_base && buf->iov_len))
		virtio_panic("bad vring_desc: %p %ld\n", buf->iov_base, buf->iov_len);

	req->total_len += buf->iov_len;
}

static struct vring_desc *
get_next_desc(struct virtio_queue *q, struct vring_desc *desc, uint16_t *idx)
{
	uint16_t	desc_idx;
 ...
 */
static int
virtio_process_one(struct virtio_dev *dev, int qidx)
{
	...
	struct vring_desc	*desc = vring_desc_at_avail_idx(q, _req.idx);
	...
	if (desc && le16toh(desc->flags) & VRING_DESC_F_NEXT)
 ...
 */
void
virtio_process_queue(struct virtio_dev *dev, uint32_t qidx)
{
	struct virtio_queue	*q = &dev->queue[qidx];
...
virtio_read_device_features(struct virtio_dev *dev)
{
...
static inline void
virtio_write_driver_features(struct virtio_dev *dev, uint32_t val)
{
	uint64_t	tmp;
...
static int
virtio_read(void *data, int offset, void *res, int size)
{
	uint32_t	val;
...
static inline void
set_ptr_low(void **ptr, uint32_t val)
{
...
static inline void
set_ptr_high(void **ptr, uint32_t val)
{
	uint64_t	tmp = (uintptr_t)*ptr;
...
static inline void
set_status(struct virtio_dev *dev, uint32_t val)
{
	if ((val & VIRTIO_CONFIG_S_FEATURES_OK) &&
	    (!(dev->driver_features & BIT(VIRTIO_F_VERSION_1)) ||
	     !(dev->driver_features & BIT(VIRTIO_RING_F_EVENT_IDX)) ||
...
static int
virtio_write(void *data, int offset, void *res, int size)
{
	struct virtio_dev	*dev = (struct virtio_dev *)data;
...
static const struct iomem_ops	virtio_ops = {
	.read = virtio_read,
	.write = virtio_write,
};
...
void
virtio_set_queue_max_merge_len(struct virtio_dev *dev, int q, int len)
{
	dev->queue[q].max_merge_len = len;
}

int
virtio_dev_setup(struct virtio_dev *dev, int queues, int num_max)
{
	...
	dev->int_status = 0;
	dev->device_features |= BIT(VIRTIO_F_VERSION_1) | BIT(VIRTIO_RING_F_EVENT_IDX);
	dev->queue = malloc(qsize);
	if (!dev->queue)
		return -ENOMEM;
	...
	dev->base = ulvisor_register_iomem(dev, mmio_size, &virtio_ops);
	if (!dev->base) {
		free(dev->queue);
		return -ENOMEM;
	}

	avail = sizeof(virtio_devs) - (devs - virtio_devs);
	...
	if (num_bytes < 0 || num_bytes >= avail) {
		...
		ulvisor_unregister_iomem(dev->base);
		free(dev->queue);
		return -ENOMEM;
	}
	devs += num_bytes;
