...
void ulvisor_launch(void *stack, void *start_fn);
...
{
	ulvisor_t	*ulvisor = (ulvisor_t *)arg;
	...
	stack = setup_stacks(stack + 16383, ulvisor->argc, ulvisor->argv);
	ulvisor_launch(stack, ulvisor->start_fn);
...
void
run_app_thread(ulvisor_t *ulvisor)
{
	start_thread(func_user_thread, 4096, ulvisor);
	futex_wait(&futex_killed);
}

void
kill_app_thread(void)
{
